    ...
    for( let i = 0 ; i< vec_len; i++ )
    {
        ...
        switch (typetag) {
            case 'd':
                ...
                break;
            case 'f':
                _n = buf.writeFloatBE( at, _n ); // returns _n + bytes written
                break;
            case 's':
                buf.write( at, _n );
...
{
    let obj = {};
    const isBundle = buf.indexOf('#bundle') == 0;
    let _n = isBundle ? OSC_HEADER_SIZE : 0;

    while( _n < buf.length )
    {
        const msg_size = isBundle ? buf.readInt32BE(_n) : buf.length;

        const addr_start = isBundle ? _n + 4 : _n;
